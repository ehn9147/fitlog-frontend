  ...
  return (
    <>
      <Dialog open={open && !showEdit} onOpenChange={onClose}>
        <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto wireframe-dialog">
          <DialogHeader>
                    ...
                    <CardContent className="pt-4">
                      <div className="flex items-start justify-between">
                        <div className="flex items-start gap-3">
                          <div className="p-2 bg-muted rounded wireframe-card">
                            <Dumbbell className="w-4 h-4" />
                          </div>
                          <div>
                            <div className="font-medium uppercase tracking-wide">{exercise.name}</div>
                            <div className="text-sm text-muted-foreground font-mono mt-1">
                              {exercise.sets} sets × {exercise.reps} reps
                              {exercise.weight && exercise.weight > 0 && ` • ${exercise.weight} lbs`}
                            </div>
                            {exercise.notes && (
                              <p className="text-sm text-muted-foreground mt-1 font-mono">{exercise.notes}</p>
                            )}
                          </div>
                        </div>
                      </div>
            ...
            {workout.notes && (
              <div>
                ...
                <Card className="wireframe-card">
                  <CardContent className="pt-4">
                    <p className="text-sm text-muted-foreground font-mono">{workout.notes}</p>
                  </CardContent>
                </Card>
              </div>
            )}
      ...
      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent className="wireframe-dialog">
          <AlertDialogHeader>
            <AlertDialogTitle className="uppercase tracking-wide font-mono">Delete Workout?</AlertDialogTitle>
            <AlertDialogDescription className="font-mono">
              This action cannot be undone. This will permanently delete your workout data.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel className="wireframe-button" data-variant="outline">Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={handleDelete} className="wireframe-button bg-destructive text-destructive-foreground">
              Delete
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
